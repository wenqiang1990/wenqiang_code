...
class Imtest(unittest.TestCase):    
    ...
    def test_login(self):   
        ...
        self.driver.find_element_by_id("activity_amin_rb3").click()
        time.sleep(2)
        self.driver.find_element_by_id("btn_login").click()
        self.driver.find_element_by_id("ed_phone").send_keys("13311267857")
        self.driver.find_element_by_id("ed_pwd").send_keys("111111")
        self.driver.press_keycode('4')
        try:
            time.sleep(1)
            self.driver.find_element_by_name(u"确认放弃登陆？")
            self.driver.find_element_by_id("cancel_button").click()
        except:
            print u"成功收回了输入法"
        
        self.driver.find_element_by_id("btn_submit").click()#点击登陆
        #强制等待
        print "Start : %s" % time.ctime()
        time.sleep(2)
        print "End : %s" % time.ctime()
        el=self.driver.find_element_by_id("yy_pcenter_userid").get_attribute("text")
        assert_equal(el, "13311267857", msg=u"登录账号验证失败")#18515617992
        print u"登录账号是+++++++++"+el+"+++++++++"
