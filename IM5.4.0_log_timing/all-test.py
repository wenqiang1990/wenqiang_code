...
def send_mail(file_new):
    ...
    mail_to=['1058099258@qq.com','wenqiang@yuntongxun.com']#收件箱
    #定义正文
    f = open(file_new, 'rb')
    mail_body = f.read()
    f.close()
...
def alltests():
    ...
    alltests = suit.Suit()
    #定义测试文件查找的目录
    test_dir='E:\\workspace\\IM5.4.0_log_timing'
    #定义 discover 方法的参数
...
if __name__ == '__main__':
    ...
    testreport = 'E:\\workspace\\appium_report\\'
    ...
    alltestnames = alltests()
    runner.run(alltestnames)
    fp.close()
    with open('F:\Appium\count\\fail.txt','r') as f:
        fail=int(f.read())
    with open('F:\Appium\count\\error.txt','r') as f:
        error=int(f.read())#需要int型     
